...
zig_quicksort = ctypes.cdll.LoadLibrary("zig-out/bin/zig_quicksort.dll")
...
class ZigImpl:
    @staticmethod
    def quicksort(data: list[float]) -> float:
        zig_data = (ctypes.c_float * len(data))(*data)
        zig_array_pointer = ctypes.cast(zig_data, ctypes.POINTER(ctypes.c_float))

        start = time.perf_counter()
        zig_quicksort.quicksort(zig_array_pointer, len(data))
        end = time.perf_counter()
